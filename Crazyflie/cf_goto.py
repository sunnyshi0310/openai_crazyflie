...
from openai_ros.robot_envs import cf_env
...
class CrazyflieGotoEnv(cf_env.CrazyflieEnv):
    def __init__(self):
        """
        ...
        #             launch_file_name="openai_crazyflie2_swarm.launch",
        #             ros_ws_abspath=ros_ws_abspath)

        # Load Params from the desired Yaml file
        LoadYamlFileParamsTest(rospackage_name="openai_ros",
                               rel_path_from_package_to_file="src/openai_ros/task_envs/Crazyflie/config",
                               yaml_file_name="cf_goto.yaml")
        ...
        super(CrazyflieGotoEnv, self).__init__(ros_ws_abspath)
    ...
    def _set_action(self, action):
        ...
        self.move_base(delta_x,delta_y,delta_z,
                       epsilon=0.05,
                       update_rate=10)
    ...
    def _get_obs(self):
        ...
        roll, pitch, yaw = self.get_orientation_euler(gt_pose.pose.pose.orientation)


        # We simplify a bit the spatial grid to make learning faster
        observations = [int(gt_pose.pose.pose.position.x),
                        int(gt_pose.pose.pose.position.y),
                        int(gt_pose.pose.pose.position.z),
                        round(roll, 1),
                        round(pitch, 1),
                        round(yaw, 1)]
